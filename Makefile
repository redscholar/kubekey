...
# executor
KK_IMAGE_NAME ?= kk-executor
KK_CONTROLLER_IMG ?= $(REGISTRY)/$(KK_IMAGE_NAME)
...
.PHONY: kk-releaser 
kk-releaser: $(GORELEASER_BIN) ## build releaser in dist. it will show in https://github.com/kubesphere/kubekey/releases
...
docker-push-kk-executor: ## Build the docker image for kk-executor
	@$(DOCKER_PUSH) \
		...
		-f build/kk/Dockerfile -t $(KK_CONTROLLER_IMG):$(TAG) .
...
docker-push-capkk-controller-manager: ## Build the docker image for capkk-controller-manager
	@$(DOCKER_PUSH) \
		--build-arg builder_image=$(GO_CONTAINER_IMAGE) \
		--build-arg goproxy=$(GOPROXY) \
		--build-arg ldflags="$(LDFLAGS)" --build-arg build_tags="cluster-api" \
		-f build/kk/Dockerfile -t $(KK_CONTROLLER_IMG):$(TAG) .
...
$(KUSTOMIZE): # Build kustomize into tools folder.
	@if [ ! -f $(OUTPUT_TOOLS_DIR)/$(KUSTOMIZE_BIN) ]; then \
...
$(GORELEASER): # Build goreleaser into tools folder.
	@if [ ! -f $(OUTPUT_TOOLS_DIR)/$(GORELEASER_BIN) ]; then \
