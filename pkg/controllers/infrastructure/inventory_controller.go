...
func (r *InventoryReconciler) SetupWithManager(mgr manager.Manager, o options.ControllerManagerServerOptions) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&clusterv1beta1.Cluster{}, handler.EnqueueRequestsFromMapFunc(r.objectToInventoryMapFunc)).
...
func (r *InventoryReconciler) Reconcile(ctx context.Context, req reconcile.Request) (_ reconcile.Result, retErr error) {
	...
	if err := scope.newPatchHelper(r.Client, scope.Inventory); err != nil {
		return ctrl.Result{}, err
	}
	defer func() {
...
func (r *InventoryReconciler) reconcileDelete(ctx context.Context, scope *clusterScope) error {
	...
	for _, obj := range kkmachineList.Items {
		if err := r.Client.Delete(ctx, &obj); err != nil {
			return err
		}
	}
	if len(pipelineList.Items) == 0 && len(kkmachineList.Items) == 0 {
		// Delete finalizer.
...
func (r *InventoryReconciler) createHostCheckPipeline(ctx context.Context, scope *clusterScope) error {
	if ok, _ := scope.ifPipelineCompleted(ctx, r.Client, scope.Inventory); !ok {
		return nil
	}
	// todo when install offline. should mount workdir to pipeline.
	volumes, volumeMounts := scope.getVolumeMounts()
	pipeline := &kkcorev1.Pipeline{
...
func (r *InventoryReconciler) syncInventoryControlPlaneGroups(ctx context.Context, scope *clusterScope) error {
	...
	for _, kkmachine := range kkmachineList.Items {
		if kkmachine.Spec.ProviderID != nil {
			existControlPlaneHosts = append(existControlPlaneHosts, *kkmachine.Spec.ProviderID)
		}
	}
	...
	f, err := getHostSelectorFunc(scope.KKCluster.Spec.HostSelectorPolicy)
	if err != nil {
		return err
	}

	return r.setProviderID(ctx, kkmachineList,
		f(ctx, getControlPlaneGroupName(), len(machineList.Items)-len(existControlPlaneHosts), scope.Inventory))
}
...
func (r *InventoryReconciler) syncInventoryWorkerGroups(ctx context.Context, scope *clusterScope) error {
	...
	for _, kkmachine := range kkmachineList.Items {
		if kkmachine.Spec.ProviderID != nil {
			existWorkerHosts = append(existWorkerHosts, *kkmachine.Spec.ProviderID)
		}
	}
	...
	return r.setProviderID(ctx, kkmachineList,
		f(ctx, getWorkerGroupName(), len(machineList.Items)-len(existWorkerHosts), scope.Inventory))
}

// setProviderID set providerID to kkmachine from inventory.groups[groupName].
// if machine already has providerID, skip.
func (r *InventoryReconciler) setProviderID(ctx context.Context, kkmachineList *capkkinfrav1beta1.KKMachineList, availableHosts []string) error {
	...
	for _, host := range availableHosts {
		for _, kkmachine := range kkmachineList.Items {
			...
			kkmachine.Spec.ProviderID = ptr.To(host)
